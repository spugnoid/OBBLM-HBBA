...
if ($_POST) {
  ...
  while (list($key, $val) = each($postvars)) {
    ...
    elseif ($key == 'Bloodweiser Kegs') { $ind_Kegs = (int) $val; continue; }
...
if (isset($ind_Kegs)) { $ind_cost += $ind_Kegs*$inducements['Bloodweiser Kegs']['cost']; }
else $ind_Kegs = '__';
...
$pdf->print_inducements(MARGINX, ($currenty+=$h), $h, COLOR_ROSTER_NORMAL, DEFLINECOLOR, 8, 'Bloodweiser Kegs (0-2):', $ind_Kegs, $pdf->Mf($inducements['Bloodweiser Kegs']['cost']));
