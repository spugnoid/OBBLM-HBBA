...
class PDFMatchReport implements ModuleInterface
{
  ...
  public static function getModuleTables(){return array();}
    ...
    public static function main($argv)
    {
        ...
        define("MARGINX", 20); //was 20
	define("MARGINY", 20);
	define("DEFLINECOLOR", '#000000');
	define("HEADLINEBGCOLOR", '#ffffff'); //was c3c3c3
	...
	$ind_cost=0;
        ...
        $pdf=new BB_PDF('L','pt','Letter'); // Creating a new PDF doc. landscape, scale=pixels, size Letter (was A4)
...
#        $pdf->RoundedRect($currentx, $currenty, 542, 20, 6, 'DF'); // Filled rectangle around Team headline
        ...
        $pdf->Cell(360, 20, 'HBBA Match Report', 0, 0, 'R', false, '');  // make a call to get the league Name instead of hardcoded
...
#        $pdf->RoundedRect($currentx + 475, $currenty + $scoreboxOffset, 50, 50, 5, 'D');
        if ($FILLED) {
            ...
            $pdf->Image($img2->getPath(),$currentx + 700, $currenty + $scoreboxOffset, 60,60,'','',false,0); //was $currentx + 720,
	...
	// Gate + score text
       ...
       //Printing headers for player rows. Do all this twice
        $smallFieldSize = 25;
        $mediumFieldSize = 70; //was 90
        $nameFieldSize = 80; //was 100
        $skillFieldSize = 330; //Was 350
	    $statFieldSize = 20; //Added to reformat printed sheets 
        ...
        while ($i < 2) {
            ...
            $pdf->Cell(50, $h, 'Winnings', 2, 0, 'C', true, '');
            $pdf->Cell(60, $h, 'Fan Factor', 2, 0, 'C', true, '');
            $pdf->Cell(70, $h, 'Total team CAS', 2, 0, 'C', true, '');
            $pdf->Cell(35, $h, 'FAME', 2, 0, 'C', true, '');
          ...
          if (1 && $FILLED) {
            ...
            $pdf->Cell($statFieldSize, $h, 'Nr', 1, 0, 'C', true, '');
            $pdf->Cell($mediumFieldSize, $h, 'Position', 1, 0, 'C', true, '');
	        $pdf->Cell($statFieldSize, $h, 'MA', 1, 0, 'C', true, '');
	        $pdf->Cell($statFieldSize, $h, 'ST', 1, 0, 'C', true, '');
	        $pdf->Cell($statFieldSize, $h, 'AG', 1, 0, 'C', true, '');
	        $pdf->Cell($statFieldSize, $h, 'AV', 1, 0, 'C', true, '');
